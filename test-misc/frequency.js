...
import {hex_cp, explode_cp} from '../ens-normalize.js/derive/utils.js';
...
// [8d0c5a67]
// small capitals
//let chars = [0x1D00,0x299,0x1D04,0x1D05,0x1D07,0xA730,0x262,0x29C,0x26A,0x1D0A,0x1D0B,0x29F,0x1D0D,0x274,0x1D0F,0x1D18,0xA7AF,0x280,0xA731,0x1D1B,0x1D1C,0x1D20,0x1D21,0x28F,0x1D22];

let chars = [0x30FB];

chars = [...new Set(chars)];
console.log('[' + chars.map(cp => `0x${cp.toString(16)}`).join(',') + ']');
...
let tally = Object.fromEntries(chars.map((cp, i) => [UNICODE.format(cp), buckets[i].length]))
...
let total = buckets.reduce((a, x) => a + x.length, 0);
...
writeFileSync(new URL(`./frequency-${hash}.json`, out_dir), JSON.stringify({tally, valid, total}, null, '\t'));
console.log(`Hash: ${hash}`);
*/

console.log(JSON.stringify(Object.fromEntries(chars.map((cp, i) => [UNICODE.format(cp), buckets[i]])), null, '\t'));